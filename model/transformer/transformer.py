...
class TransformerEncoderLayer(Module):
    ...
    def __init__(self, num_layer: int, d_model: int, nhead: int, dim_feedforward: int = 2048, dropout: float = 0.1,
                 activation: Union[str, Callable[[Tensor], Tensor]] = F.relu,
                 layer_norm_eps: float = 1e-5, batch_first: bool = False, norm_first: bool = False,
                 bias: bool = True, device=None, dtype=None) -> None:
        factory_kwargs = {'device': device, 'dtype': dtype}
        ...
        self.residual_attn = admin_torch.as_module(2 * num_layer)
        ...
        self.residual_ffn = admin_torch.as_module(2 * num_layer)
    ...
    def forward(
        ...
        if self.norm_first:
            ...
            x = self.norm1(
                self.residual_attn(x, self._sa_block(x, src_mask, src_key_padding_mask, is_causal=is_causal)))
            x = self.norm2(self.residual_ffn(x, self._ff_block(x)))
        return x
...
class TransformerDecoderLayer(Module):
    ...
    def __init__(self, num_layers: int, d_model: int, nhead: int, dim_feedforward: int = 2048, dropout: float = 0.1,
        ...
        self.residual_attn2 = admin_torch.as_module(2 * num_layers)

        self.multihead_attn = MultiheadAttention(d_model, nhead, dropout=dropout, batch_first=batch_first,
                                                 bias=bias, **factory_kwargs)

        self.residual_attn = admin_torch.as_module(2 * num_layers)
        # Implementation of Feedforward model
        self.linear1 = Linear(d_model, dim_feedforward, bias=bias, **factory_kwargs)
        self.dropout = Dropout(dropout)
        self.linear2 = Linear(dim_feedforward, d_model, bias=bias, **factory_kwargs)
        self.residual_ffn = admin_torch.as_module(2 * num_layers)
    ...
    def forward(
        ...
        if self.norm_first:
            ...
            x = self.norm1(self.residual_attn(x, self._sa_block(x, tgt_mask, tgt_key_padding_mask, tgt_is_causal)))
            x = self.norm2(self.residual_attn2(x, self._mha_block(x, memory, memory_mask, memory_key_padding_mask,
                                                                  memory_is_causal)))
            x = self.norm3(self.residual_ffn(x, self._ff_block(x)))
