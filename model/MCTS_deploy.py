...
class MCTS:
    ...
    def init_env(self, data):
        case_idx, items, sheets, n_typ_item, n_typ_sheet, edge, labels = data
        input_seq = items[:, :, :3]
        sheet = sheets[:, :, :3]

        self.n_typ_item = torch.tensor(n_typ_item)
        self.n_typ_sheet = torch.tensor(n_typ_sheet)

        batch_size = 1
        it_mask = torch.zeros(batch_size, self.model.max_item_type + 1)  # add begin node
        ...
        self.edge = torch.tensor(edge).to(self.gpu_id, non_blocking=True)
        self.it_mask = it_mask
    ...
    def expandsion(self, node: mctNode, expand_num: int):
        ...
        with torch.no_grad():
            target_decodes = self.model.forward_decode(target_seq, self.item_size_embedding, self.context_embeding,
                                                       self.end_embedding, self.bin_size_embedding, self.it_mask,
                                                       it_records)
        ...
        for s in infos:
            ...
            if bid != 0:
                ...
                c_env.layouts[-1].bin_tree.tune_tree_change()
                c_env.layouts[-1].bin_tree.tune_space()
                c_env.new_bin(bid - 2)
        ...
        if len(node.children) == 0:
            ...
            e.layouts[-1].bin_tree.tune_tree_change()
            e.layouts[-1].bin_tree.tune_space()
            e.new_bin(0)
            child = mctNode(state=e, parent=node)  # 只放左边
            if node.child_not_exist(child):
                rank += 1
    ...
    def evaluate(self, node):
        ...
        self.model.eval()
        env, cost_chain = self.model.forward_once(env, self.item_size_embedding, self.context_embeding,
                                                  self.bin_size_embedding,
                                                  self.end_embedding, self.max_depth)
